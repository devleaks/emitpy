        ...
        if len(Airport._DB) < 100:
            ...
            for a in csvdata:
                if a["ident"] not in Airport._DB:
                    ...
                    apt = AirportBase(icao=a["ident"], iata=a["iata_code"], name=a["name"], city=a["municipality"], country=a["iso_country"], region=a["iso_region"], lat=float(a["latitude_deg"]), lon=float(a["longitude_deg"]), alt=elev, data=a)
                    cnt += 1
            file.close()
            logger.debug("AirportBase::load: %d airports added", cnt)
...
class Airport(AirportBase):
    """
    An Airport is an airport for which we manage more details like operating airlines, routes, etc.
    ...
    def __init__(self, icao: str):
        ...
        file.close()
        self._rawdata = a
        AirportBase.__init__(self, icao=a["icao"], iata=a["iata"], name=a["name"], city=a["city"], country=a["country"], region=a["region"], lat=a["latitude"], lon=a["longitude"], alt=a["altitude"], data=a)
...
""" *********************************************************************** """
class ManagedAirport(AirportBase):
    """
    ...
    def __init__(self, icao: str, iata: str, name: str, city: str, country: str, region: str, lat: float, lon: float, alt: float, data: object):
        Airport.__init__(self, icao=icao, iata=iata, name=name, city=city, country=country, region=region, lat=lat, lon=lon, alt=alt, data=data)
        self.runways = {}
        self.taxiways = Graph()
        self.parkings = FeatureCollection()
    ...
    def load(self):
        """
        Chains loadRunways, loadParkings, loadTaxiways, loadServiceRoads, and loadProcedures
        """
