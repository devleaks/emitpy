...
class ManagedAirport:
    ...
    def cacheToRedis(self):
        ...
        _redis.set(keyname+"-airline-icao", pickle.dumps(Airline._DB))
        _redis.set(keyname+"-airline-iata", pickle.dumps(Airline._DB_IATA))
    ...
    def loadFromRedis(self):
        ...
        ret = _redis.get(keyname+"-airport-icao")
        Airport._DB = pickle.loads(ret)
        ret = _redis.get(keyname+"-airport-iata")
        Airport._DB_IATA = pickle.loads(ret)

        ret = _redis.get(keyname+"-airline-icao")
        Airline._DB = pickle.loads(ret)
        ret = _redis.get(keyname+"-airline-iata")
        Airline._DB_IATA = pickle.loads(ret)

        ret = _redis.get(keyname+"-actype-iata")
        AircraftType._DB = pickle.loads(ret)
        ret = _redis.get(keyname+"-actype-perf")
        AircraftPerformance._DB_PERF = pickle.loads(ret)
