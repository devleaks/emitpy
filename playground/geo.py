...
class Line(Feature):
    ...
    def length(self):
        return distance(self.start, self.e)
...
class BoundingBox(Polygon):
    ...
    def __init__(self, ll, ur):
        ul = Point(ur.lat, ll.lon)
        lr = Point(ll.lat, ur.lon)
        Polygon.__init__(self, [ll, ul, ur, lr, ll])
        self.ll = ll
        self.ur = ur
...
def removeShortSegments(segments, minlength=50):
    # try to remove segments shorter than minlength (in meter)
    # if they are aligned with the next one
    def inline(s1, s2, maxturn=10):
        # degrees
        return abs(s1.bearing() - s2.bearing()) < maxturn

    newsegments = []
    last = segments[0]

    for i in range(1, len(segments)):
        # do we have to remove the previous one?
        currsegment = segments[i]

        if last.length() < minlength:
            # is it inline with current one?
            if inline(last, currsegment):
                # we can remove it
                newsegment = copy.deepcopy(currsegment)
                newsegment.start = last.start
                newsegments.append(newsegment)
            else:
                # We cannot merge them
                newsegments.append(last)
...
def niceRunwayAlign(ls, destination):
    # last point is runway's start.
    # add a last segment from runway's start towards runway's end.
    newls = []
    return newls


def niceParking(ls, destination):
    # last point is end of segment with closest point to parking.
    # last point may be beyond parking.
    # So find closest point on taxiway to parking.
    # Either add segment from last point of ls to closest point on taxiways
    # Or remove last segment and add new segment from "start of that last segment we just removed" to closest point on taxiways.
    # Finally, add segment from closest point on taxiways to parking (straight line).
    newls = []
    return newls


def niceEnd(ls, destination, move):
    # Tries to make a last smooth segment to destination
    if move == "departure":
        return niceRunwayAlign(ls, destination)
    return niceParking(ls, destination)



def niceParkingLeave(ls, position):
    # Inverse of niceParking
    newls = []
    return newls


def niceRunwayLeave(ls, position):
    # Tries to find a runway exit to taxiway in front of the plane (so that it does not U-turn)
    # Tries to invite plane to take next exit (if speed reasonable (< 40km/h))
    newls = []
    return newls


def niceStart(ls, position, move):
    # Tries to make a last smooth segment to destination
    if move == "departure":
        return niceParkingLeave(ls, position)
    return niceRunwayLeave(ls, position)
...
def fullSmoothing(ls, data, move):
    noss = removeShortSegments(ls)
    ns = niceStart(noss, data, move)
    ne = niceEnd(ns, data, move)
    return smoothTurns(ne)
