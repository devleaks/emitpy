...
class Graph:  # Graph(FeatureCollection)?
        ...
        if len(options) > 0:
            ...
            for dst in src.adjacent.keys():
                ...
                if "bbox" in options:
                    ...
                    bbOk = pointInPolygon(dstp, options["bbox"])
                # logging.debug("%s %s %s %s %s" % (dst, v.usage, code, txyOk, scdOk))
                if bbOk and txyOk and scdOk:
    ...
    def add_edge(self, edge):
        if edge.start.id in self.vert_dict and edge.end.id in self.vert_dict:
            ...
            if(edge.direction == TWOWAYS):
                self.vert_dict[edge.end.id].add_neighbor(self.vert_dict[edge.start.id].id, edge.cost)
        else:
            logging.critical("Graph::add_edge: vertex not found when adding edges %s,%s", edge.start.id, edge.end.id)
    ...
    def get_edge(self, src, dst):
        ...
        arr = list(filter(lambda x: x.start.id == dst and x.end.id == src and x.direction == TWOWAYS, self.edges_arr))
        if len(arr) > 0:
            return arr[0]
    ...
    def get_vertices(self, bbox = None):
        if bbox is not None:
            return list(map(lambda x: x.id, self.findVertexInPolygon(bbox)))
