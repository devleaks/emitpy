...
class CreateFlight(BaseModel):
    ...
    is_cargo: bool = False
    load_factor: Optional[float] = Field(1.0, description="Load factor for flight, 0 is empty, 1 is full")
    flight_actual_date: Optional[date] = Field(None, description="Flight actual date in managed airport local time")
    flight_actual_time: Optional[time] = Field(None, description="Flight actual time in managed airport local time")
