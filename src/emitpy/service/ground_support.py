...
class RSTSchedule:
    ...
    def getStartEndTimes(self, timetype: str = "scheduled"):
        ...
        if timetype == "estimated" and self.estimated is not None:
            return (
                self.estimated + timedelta(minutes=self.reltime),
                self.estimated + timedelta(minutes=(self.reltime + self.duration)),
            )
        return (
            self.scheduled + timedelta(minutes=self.reltime),
    ...
    def getWarnAlertTimes(self, timetype: str = "scheduled"):
        if timetype == "actual":
            return (
                self.actual_start + timedelta(minutes=self.warn),
                self.actual_start + timedelta(minutes=self.alert),
            )
        if timetype == "estimated" and self.estimated is not None:
            return (
                self.estimated + timedelta(minutes=self.warn),
                self.estimated + timedelta(minutes=self.alert),
            )
        return (
            self.scheduled + timedelta(minutes=self.warn),
...
class GroundSupport(Messages):
    def __init__(
        self, operator: "Company", scheduled: int = 0, duration: int = 0, **kwargs
    ):
    ...
    def setRSTSchedule(
        self, relstartime: int, duration: int, warn: int = None, alert: int = None
    ):
    ...
    def terminated(
        ...
        self.schedule_history.append(
            (dt.isoformat(), "TT", info_time.isoformat())
        )  # Terminated Time
