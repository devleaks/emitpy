...
from emitpy.parameters import XPLANE_DIR, TEMP_DIR
...
class XPAirportWeather(AirportWeather):
	...
	def init(self):
		...
		if self.source_date is None:
			self.parsed_metar = Metar.Metar(self.metar)
		else:
			self.parsed_metar = Metar.Metar(self.metar, month=self.source_date.month, year=self.source_date.year)
		# if self.parsed_metar is not None:
		# 	logger.debug(self.parsed_metar.string())
	...
	def summary(self):
		...
		return self.parsed_metar.string()
	...
	def get_wind(self) -> Wind:
		if self.parsed_metar is None:
			return None
		wind_dir = self.parsed_metar.wind_dir.value() if self.parsed_metar.wind_dir is not None else None
		wind_speed = self.parsed_metar.wind_speed.value(units="MPS") if self.parsed_metar.wind_speed is not None else None
		return Wind(direction=wind_dir, speed=wind_speed)

	def get_precipirations(self) -> float:
		if self.parsed_metar is not None:
			if self.parsed_metar.precip_1hr is not None:
				if self.parsed_metar.precip_1hr.istrace():
					return 0.1
				return self.parsed_metar.precip_1hr.value(units="CM")
		return 0
...
class XPWeatherEngine(WeatherEngine):
	...
	def parse_grib_data(self, lat, lon, alt, moment):
		...
		for line in iter(p.stdout):
			...
			level, variable, value, lat, lon = [r[4].split(" "), r[3], r[7].split(",")[2].split("=")[1], r[7].split(",")[1].split("=")[1], r[7].split(",")[0].split("=")[1]]
			if plat is None:
		...
		for level, wind in winds.items():
			if "UGRD" in wind and "VGRD" in wind:
				lvl = level
				if "-" in level:	  # if it is a Layer between two specified height levels: Height of top in hm - Height of bottom in hm above ground: Example: 30-0
					wa = level.split("-")
					if len(wa) > 0:
