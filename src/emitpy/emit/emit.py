...
class Emit(Movement):
    ...
    def emit(self, frequency: int):
        ...
        def emit_point(idx, pos, time, reason, waypt=False):
            ...
            e.setProp(
                FEATPROP.EMIT_INDEX.value, len(self.getEmitPoints())
            )  # Sets unique index on emit features
            e.setProp(FEATPROP.BROADCAST.value, not waypt)
            if (
                self.emit_type == "service" and e.getMark() is not None
            ):  # and e.getMark() is not None:
                logger.debug(
                    f"added mark={e.getMark()}, reason={reason}, emit={e.getProp(FEATPROP.BROADCAST.value)}"
                )
        ...
        while curridx < (len(self.move_points) - 1):
            ...
            if emit_details:
                logger.debug(
                    f">>> new vertex: {curridx}, e={len(self.getEmitPoints())} s={self.move_points[curridx].speed()}"
                )
    ...
    def getTimedMarkList(self):
        ...
        for f in self._scheduled_points:
            ...
            if m is not None:
                ...
                line.append(
                    datetime.fromtimestamp(f.getProp(FEATPROP.EMIT_ABS_TIME.value))
                    .astimezone()
    ...
    def addToPause(self, sync, duration: float, add: bool = True):
        ...
        if f is not None and len(f) > 0:
            ...
            before = (
                r.getProp(FEATPROP.PAUSE.value)
                if (r.getProp(FEATPROP.PAUSE.value) is not None and add)
                else 0
            )
            r.setPause(before + duration)
            logger.debug(
                f"found {sync} mark, added {duration} sec. pause for a total of {r.getProp(FEATPROP.PAUSE.value)}"
            )
