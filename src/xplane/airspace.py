...
from .geo import Point, BoundingBox, destination
...
class Navpoint(Vertex):
    ...
    def __init__(self, ident, region, airport, lat, lon, navtype):
        Vertex.__init__(self, Navpoint.mkId(region, airport, ident, navtype), Point(lat, lon))
        self.ident = ident
        self.region = region
...
class AirwaySegment(Edge):
    ...
    def __init__(self, names, start, end, direction, lowhigh, fl_floor, fl_ceil):
        Edge.__init__(self, start, end, direction)
...
class Airspace(Graph):
    ...
    def loadAirports(self):
        ...
        count = 0
        filename = os.path.join(SYSTEM_DIRECTORY, "Resources", "GlobalAirportDatabase.txt")
    ...
    def mkRoute(self, src, dst):
        ...
        self.bbox = self.mkBbox(asrc, adst, 150000)

        route = Route(self, vsrc[0], vdst[0], "", {})
        route.find()
