...
class Broadcaster:
    ...
    def broadcast(self):
        ...
        try:
            while True:
                logger.debug(f":run: listening on queue {self.name}..")
            ...
            if nv is not None:
                ...
                logger.debug(f":run: keyboard interrupt, nothing to push back on queue")
        finally:
            logger.debug(f":run: quitting..")
            self.redis.publish("Q"+self.name, "quit")
            logger.debug(f":run: ..waiting for trimmer..")
            self.trim.join()
